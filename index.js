...
app.put("/addmentor/:name", async function (req, res) {
  ...
  const result = await client
    .db("task42")
    .collection("mentors")
    .updateMany({ name: name }, { $set: { students: data } });
  data.map(async (student) => {
    let result = await client
      .db("task42")
      .collection("students")
      .updateMany({ name: student }, { $set: { mentor: name } });
  });
  res.send(result);
});
...
app.put("/changementor/:name", async function (req, res) {
  ...
  const result = await client
    .db("task42")
    .collection("students")
    .updateOne({ name: name }, { $set: { mentor: req.body.mentor } });
  res.send(result);
});
...
app.get("/getstudent/:id", async function (req, res) {
  ...
  const data = await client
    .db("task42")
    .collection("mentors")
    .find({ _id: ObjectId(id) }, { students: 1 })
    .toArray();
  console.log(data);
  res.send(data);
});
